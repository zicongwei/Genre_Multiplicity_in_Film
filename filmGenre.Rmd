...
plot2 <- df2 %>%
  ...
  geom_line() +
  ggtitle("Proportion of Short, and Comedy in Musical") +
  theme_fivethirtyeight() +
...
other_genres <- df_usa %>%
  filter((genre == 'Romance' | genre == 'Comedy' | genre == 'Western' | genre == 'Animation' | genre == 'Western')) %>%
  group_by(id) %>%
  summarise(production_year = first(production_year))

df4 <- df_usa %>%
  #filter(id %in% short$id) %>%
  #filter(genre != 'Western' & genre != 'Short') %>%
  filter(id %in% short$id & id %in% other_genres$id) %>%
  filter(genre == 'Romance'  | genre == 'Western' | genre == 'Animation' | genre == 'Comedy') %>%
  group_by(production_year, genre) %>%
  summarize(count = n())
